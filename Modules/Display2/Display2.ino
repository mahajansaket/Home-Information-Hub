...
const char *wifiSsid = "WiFi-SSID"; // change this to the ssid of the network
const char *wifiPassword =  "****"; // change this to the password of the network
...
void parseData()
{
  ...
  tft.println(&timeinfo, "%A, %B %d %Y %H:%M:%S"); //Display Time

  // Display Current Weather

  String response = returnFilteredResponse("http://smahajan.com/current_weather");
  ...
  roomTemp = (volts - 0.5) * 100 ;         //calculate temperature celsius from voltage as per the equation found on the sensor spec sheet.

  tft.println("Room Temperature:");
  tft.println(roomTemp);

  //    Weather Forecast

  String response1 = returnFilteredResponse("http://smahajan.com/weather_forecast");

  JsonObject& forecastJson = jsonBuffer.parseObject(response1);
  Serial.println("Inside Parse Data");

  const char* descWeather = forecastJson["description"];
  char* weatherForecast = (char*) descWeather;
  tft.println("Weather Forecast:");
  tft.println(weatherForecast);


  //Maps showing ETA

  String response3 = returnFilteredResponse("http://smahajan.com/maps");
